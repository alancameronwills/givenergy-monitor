...
`src/modbus/client.js` retries each register batch up to `READ_RETRIES` (2) times with a `RETRY_DELAY_MS` (750 ms) gap before reporting a read as failed, so brief drops don't surface as an outage.

`src/routes/read.js` runs a background reconnect monitor (`setInterval`, 60 s). Once any modbus read has succeeded (`hasConnected = true`), it triggers a rescan when either reads have been stale for 20 minutes (fallback when nothing is polling) **or** `consecutiveFailures >= FAST_RESCAN_FAILURES` (3) — i.e. an actively-polling client has seen ~3 failed reads in a row. Before scanning it calls `hasLanConnectivity()` (in `src/scanner.js`) to check whether the Pi can reach its default gateway. If the LAN is unreachable (WiFi dropout), the cycle is skipped without advancing `nextScanAt`, so a scan fires immediately once connectivity returns. If the LAN is up but the inverter is gone, it runs a network scan and updates `config.host` live. On scan failure it retries every 20 minutes.

`refreshCache()` tracks diagnostic state (`consecutiveFailures`, `lastLanUp`, `lastError` with a `layer` of `wifi`/`inverter`). `GET /status` exposes this so the UI can distinguish a Pi WiFi dropout from a lost inverter link; `public/pv.html` fetches it (`diagnose()`) to label the outage banner.
